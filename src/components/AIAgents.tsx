...
export const AIAgents: React.FC<AIAgentsProps> = ({ isOpen, onClose, metrics, learners, attendanceData, practiceData, projectData }) => {
  ...
  const handleAuraQuery = async (e: React.FormEvent) => {
    ...
    INSTRUCTIONS: 
    - Search the Learner Database rigorously.
  ...
  const handleNexusCheck = async () => {
    ...
    Provide a professional summary of the hardest-hit cohorts/modules and suggest 2 direct, actionable steps.`;
  ...
  const handleNovaOutreach = async () => {
    ...
    if (targets.length === 0) {
      setAgentOutput(`✅ Automated Scan Complete: No learners in Cohort ${novaTargetCohort} were found with critically low practice submissions and valid email addresses.`);
      setIsLoading(false);
      return;
    }
    ...
    Write a short, highly personalized email for each of these students falling behind in Cohort ${novaTargetCohort}:
    
    ${contextStr}
    
    Return the result EXACTLY as a valid JSON array of objects with "name", "subject", and "body" keys. Do NOT use markdown code blocks.`;
    ...
    try {
      ...
      setAgentOutput("⚠️ Intervention failed. The neural core returned an unparseable communication string.\n\nRaw Output:\n" + response);
    }
  ...
  const handleJudgeEvaluation = async () => {
    ...
    Write a highly dramatic, strict, 3-sentence verdict on their performance. Praise the fastest/most efficient agent, and ruthlessly mock the inactive or slow ones.`;
            ...
            >
              ...
              <div className="md:hidden h-16 border-b border-slate-800 flex items-center justify-between px-4 bg-slate-950 z-30 shrink-0">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-black text-white tracking-widest uppercase">Neural Core</h2>
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left p-4 rounded-2xl border transition-all duration-300 flex items-center gap-4 ${
                          isActive ? `${agent.bg} ${agent.border} shadow-lg` : 'border-transparent hover:bg-slate-800/50 text-slate-400 hover:text-slate-200'
                        }`}
                      >
              ...
              <div className="flex-1 flex flex-col bg-slate-900 relative overflow-hidden h-full">
                <div className="hidden md:flex h-20 border-b border-slate-800 px-8 items-center justify-between bg-slate-900/50 backdrop-blur-md z-10 shrink-0">
                  ...
                  <button onClick={onClose} className="p-2 text-slate-500 hover:text-white hover:bg-slate-800 rounded-full transition-colors">
                    <X className="w-6 h-6" />
                  </button>
                </div>
                    ...
                    <div className="flex flex-col h-full">
                      <div className="flex-1 space-y-6 overflow-y-auto pb-4 pr-2 scrollbar-elegant">
                        ...
                        {chatHistory.map((msg, i) => (
                          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[90%] md:max-w-[85%] p-4 rounded-2xl text-sm whitespace-pre-wrap leading-relaxed ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-slate-800 text-slate-200 rounded-bl-none border border-slate-700 shadow-lg'}`}>
                              {msg.text}
                            </div>
                          </div>
                        ))}
                        {isLoading && (
                          <div className="flex justify-start">
                            <div className="p-4 bg-slate-800 rounded-2xl rounded-bl-none border border-slate-700 flex items-center gap-2">
                              ...
                              <span className="text-slate-400 text-sm">Executing deep scan...</span>
                            </div>
                          </div>
                        )}
                        <div ref={chatEndRef} />
                      ...
                      <form onSubmit={handleAuraQuery} className="mt-auto relative pt-4 shrink-0">
                        <input 
                          ...
                          className="w-full bg-slate-800 border border-slate-700 text-white rounded-2xl py-4 pl-4 pr-14 md:pl-6 md:pr-16 focus:outline-none focus:border-cyan-500 transition-colors shadow-inner text-sm md:text-base"
                        />
                      ...
                      {!agentOutput && !isLoading ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-center space-y-6">
                          <div className="w-20 h-20 md:w-24 md:h-24 bg-emerald-500/10 rounded-full flex items-center justify-center border border-emerald-500/20">
                            <UserCheck className="w-10 h-10 md:w-12 md:h-12 text-emerald-400" />
                          </div>
                          <div>
                            <h3 className="text-xl md:text-2xl font-bold text-white mb-2">Automated Attendance Scan</h3>
                            <p className="text-slate-400 text-sm md:text-base max-w-md mx-auto">Nexus scans raw records to pinpoint exactly who is failing attendance and maps out an intervention strategy.</p>
                          </div>
                          <button onClick={handleNexusCheck} className="px-6 py-3 md:px-8 md:py-4 bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-bold rounded-2xl shadow-lg shadow-emerald-500/20 flex items-center gap-2 transition-all hover:scale-105">
                            <Zap className="w-5 h-5" /> Initiate Diagnostic
                          </button>
                        </div>
                      ) : (
                        <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-4 md:p-6 text-slate-200 text-sm md:text-base whitespace-pre-wrap leading-relaxed shadow-inner overflow-y-auto h-full scrollbar-elegant">
                          {isLoading ? (
                            <div className="flex flex-col items-center justify-center h-full gap-4 text-emerald-400">
                              <Loader2 className="w-8 h-8 animate-spin" />
                              <p className="font-bold uppercase tracking-widest text-xs md:text-sm">Processing Diagnostics...</p>
                            </div>
                          ) : (
                  ...
                  {activeAgent === 'Nova' && (
                    <div className="h-full flex flex-col">
                      {!isLoading && draftedEmails.length === 0 && !agentOutput ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-center space-y-6">
                          <div className="w-20 h-20 md:w-24 md:h-24 bg-rose-500/10 rounded-full flex items-center justify-center border border-rose-500/20">
                            <Mail className="w-10 h-10 md:w-12 md:h-12 text-rose-400" />
                          </div>
                          <div>
                            <h3 className="text-xl md:text-2xl font-bold text-white mb-2">Smart Email Outreach</h3>
                            <p className="text-slate-400 text-sm md:text-base max-w-md mx-auto">Nova searches active cohorts for missing submissions, drafts personalized intervention emails, and connects directly to your Mail client.</p>
                          </div>
                          
                          <div className="flex flex-col items-center gap-2 bg-slate-800 border border-slate-700 p-4 rounded-2xl mt-2 w-full max-w-xs">
                            ...
                            <select 
                              ...
                              className="w-full bg-slate-900 border border-slate-700 text-white text-sm rounded-lg px-4 py-2 outline-none focus:border-rose-500 cursor-pointer"
                            >
                          ...
                          <button onClick={handleNovaOutreach} disabled={!novaTargetCohort} className="w-full max-w-xs py-4 bg-rose-500 hover:bg-rose-400 disabled:opacity-50 disabled:cursor-not-allowed text-slate-900 font-bold rounded-2xl shadow-lg shadow-rose-500/20 flex items-center justify-center gap-2 transition-all hover:scale-105">
                        ...
                        <div className="flex-1 overflow-y-auto pr-2 scrollbar-elegant space-y-4">
                          {isLoading ? (
                            <div className="flex flex-col items-center justify-center h-full gap-4 text-rose-400">
                              <Loader2 className="w-8 h-8 animate-spin" />
                              <p className="font-bold uppercase tracking-widest text-xs md:text-sm">Synthesizing Communications...</p>
                            </div>
                          ) : (
                            <>
                              {agentOutput && (
                                 <div className="p-4 bg-slate-800 border border-slate-700 text-white text-sm md:text-base rounded-xl mb-4 whitespace-pre-wrap">
                                    {agentOutput}
                                 </div>
                              )}
                              {draftedEmails.map((email, i) => (
                                <div key={i} className="bg-slate-800 border border-slate-700 rounded-2xl p-4 md:p-6 shadow-lg">
                                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-4">
                                    <div>
                                      <h4 className="text-base md:text-lg font-bold text-white">{email.name}</h4>
                                      <p className="text-[10px] md:text-xs text-rose-400 font-mono">{email.email}</p>
                                    </div>
                                    <span className="w-fit px-3 py-1 bg-rose-500/10 border border-rose-500/30 text-rose-400 text-[10px] uppercase font-bold tracking-widest rounded-lg">
                                      {email.module}
                                    </span>
                                  </div>
                                  <div className="bg-slate-900 rounded-xl p-3 md:p-4 mb-4">
                                    <p className="text-xs md:text-sm font-bold text-slate-300 mb-2 border-b border-slate-800 pb-2">Subj: {email.subject}</p>
                                    <p className="text-xs md:text-sm text-slate-400 whitespace-pre-wrap leading-relaxed">{email.body}</p>
                                  </div>
                                  <a 
                                    href={`mailto:${email.email}?subject=${encodeURIComponent(email.subject)}&body=${encodeURIComponent(email.body)}`}
                                    target="_blank"
                                    rel="noreferrer"
                                    className="w-full py-3 bg-rose-600 hover:bg-rose-500 text-white font-bold text-sm md:text-base rounded-xl flex items-center justify-center gap-2 transition-colors"
                                  >
                                    <ExternalLink className="w-4 h-4" /> Transmit via Mail
                                  </a>
                                </div>
                              ))}
                  ...
                  {activeAgent === 'Judge' && (
                    <div className="h-full flex flex-col overflow-y-auto scrollbar-elegant pr-1 md:pr-2">
                      <div className="bg-slate-950 border border-amber-500/30 rounded-3xl p-4 md:p-8 shadow-[0_0_30px_rgba(245,158,11,0.1)] mb-6 shrink-0">
                        <h3 className="text-base md:text-lg font-black text-white uppercase tracking-widest mb-6 flex items-center gap-2">
                          <ShieldCheck className="text-amber-500 w-5 h-5 md:w-6 md:h-6" /> Live Efficiency Leaderboard
                        </h3>
                        <div className="space-y-3 md:space-y-4">
                          {getRealLeaderboard().map((stat, i) => (
                            <div key={stat.name} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 md:p-4 bg-slate-900 rounded-2xl border border-slate-800 gap-3 md:gap-4">
                              <div className="flex items-center gap-3 md:gap-4">
                                <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-slate-800 flex items-center justify-center font-black text-slate-400 text-xs md:text-sm">
                                  #{i + 1}
                                </div>
                                <div>
                                  <h4 className="font-bold text-white text-sm md:text-base">{stat.name}</h4>
                                  <p className="text-[10px] md:text-xs text-slate-500">Tasks Executed: <span className="text-amber-400 font-bold">{stat.runs}</span></p>
                                </div>
                              </div>
                              <div className="flex items-center gap-4 md:gap-6 text-right w-full sm:w-auto justify-between sm:justify-end">
                                <div>
                                  <p className="text-[9px] md:text-[10px] text-slate-500 uppercase font-bold flex items-center justify-start sm:justify-end gap-1"><Clock className="w-3 h-3" /> Avg Latency</p>
                                  <p className="font-mono text-white text-xs md:text-sm">{stat.speed}</p>
                                </div>
                                <div>
                                  <p className="text-[9px] md:text-[10px] text-slate-500 uppercase font-bold flex items-center justify-start sm:justify-end gap-1"><Zap className="w-3 h-3" /> Success Rate</p>
                                  <p className="font-mono text-emerald-400 font-bold text-xs md:text-sm">{stat.eff}</p>
                                </div>
                              </div>
                            </div>
                          ))}
                      ...
                      <div className="shrink-0 min-h-[120px] md:min-h-[150px] bg-amber-500/5 border border-amber-500/20 rounded-3xl p-4 md:p-6 relative flex flex-col justify-center items-center text-center">
                        {!agentOutput && !isLoading ? (
                          <button onClick={handleJudgeEvaluation} className="px-4 py-3 md:px-6 md:py-3 bg-amber-500/20 hover:bg-amber-500/30 text-amber-400 border border-amber-500/50 rounded-xl font-bold transition-colors shadow-lg shadow-amber-500/10 text-sm md:text-base">
                            Compute System Verdict
                          </button>
                        ) : isLoading ? (
                          <div className="flex flex-col items-center gap-3 md:gap-4 text-amber-500 py-2 md:py-4">
                            <Loader2 className="w-6 h-6 md:w-8 md:h-8 animate-spin" />
                            <p className="font-bold uppercase tracking-widest text-[10px] md:text-xs">Evaluating Operations...</p>
                          </div>
                        ) : (
                          <div className="space-y-3 md:space-y-4">
                            ...
                            <p className="text-amber-100/90 text-sm md:text-lg leading-relaxed italic max-w-2xl font-serif">
                              "{agentOutput}"
                            </p>
                          </div>
                        )}
